...
class Ghost:
    ...
    def move_clyde(self, depth=3):
        # Current position of clyde in grid coordinates
    ...
    def move_inky(self):
        ...
        # Move the ghost to the next position if valid
        self.x_pos += (next_move[0] - ghost_pos[0]) * 0.5#self.speed # Assuming id corresponds to the ghost's index
        self.y_pos += (next_move[1] - ghost_pos[1]) * 0.5#self.speed
        #self.x_pos=next_move[0]
        #self.y_pos=next_move[1]
        print("updated next move ",next_move[0],next_move[1])
...
while run:
    ...
    if (time.time()-st1>30 and abs((player_y//15)-clyde_xc)==1 and abs((player_x//20)-clyde_yc)==1):
        ...
        en=time.time()
        break
    if (time.time()-st1>60 and abs((player_y//15)-inky_xc)==1 and abs((player_x//20)-inky_yc)==1):
