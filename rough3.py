...
class Ghost:
    ...
    def move_inky(self):
        # Get Pacman's position and the current position of Inky
        pacman_pos = (player_y // 15, player_x // 20)
        current_pos = (self.x_pos, self.y_pos)

        # Get adjacent positions of Inky
        adjacency_dict = self.createAdjacencyDict()
        adjacent_positions = adjacency_dict.get(current_pos, [])

        # Randomly decide whether to act "dumb"
        if random.random() < 0.6:  # 50% chance to move randomly
            ...
            for neighbor in adjacent_positions:
                ...
                # Make a dumb decision: select a move even if it doesn't improve distance
                if best_move is None or random.random() < 0.6:  # 50% chance to choose current neighbor
                    best_move = neighbor
                elif distance<min_distance:
                    min_distance=distance
                    best_move=neighbor

        # Update Inky's position with the selected best move, slowed by SPEED_FACTOR
        if best_move:
            self.x_pos += (best_move[0] - current_pos[0]) * 0.5
            self.y_pos += (best_move[1] - current_pos[1]) * 0.5
            current_pos = best_move

        # Draw Inky at the new position
        self.draw()
